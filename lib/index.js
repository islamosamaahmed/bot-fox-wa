...
async function isSudo(userId) {
    try {
        const data = loadUserGroupData();
        const isOwner = settings.ownerNumber && userId && userId.startsWith(settings.ownerNumber);
        const isSudoUser = data.sudo && data.sudo.includes(userId);
        return isOwner || isSudoUser;
    } catch (error) {
